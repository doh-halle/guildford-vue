defmodule GuildfordVueWeb.ExamCentre.ExamCentreVerifyOtpLive do
  @moduledoc """
  Sprint 11.5 Slice 7 — GET /examcenter/verify-otp. Mirror of
  the candidate variant; POSTs to
  `ExamCentreSessionController.verify_otp/2`.
  """
  ...
  def mount(_params, session, socket) do
    case PendingMfa.read(session) do
      {:ok, :exam_centre, _challenge_id, _subject_id} ->
        {:ok,
         socket
  ...
  def render(assigns) do
    ...
    <main class="mx-auto max-w-md px-6 py-16 sm:py-24">
      ...
      <.form
        for={@form}
        id="exam-centre-verify-otp-form"
        action={~p"/examcenter/verify-otp"}
        method="post"
