defmodule GuildfordVueWeb.Candidate.CandidateVerifyOtpLive do
  @moduledoc """
  Sprint 11.5 Slice 7 — GET /candidate/verify-otp. Renders the
  6-digit code form. POSTs to
  `CandidateSessionController.verify_otp/2`.

  The challenge id + subject id are session-stashed by the
  prior login POST (see `GuildfordVueWeb.Auth.PendingMfa`); this
  LV is purely a form host.
  """
  ...
  def mount(_params, session, socket) do
    case PendingMfa.read(session) do
      {:ok, :candidate, _challenge_id, _subject_id} ->
        {:ok,
         socket
  ...
  def render(assigns) do
    ...
    <main class="mx-auto max-w-md px-6 py-16 sm:py-24">
      ...
      <.form
        for={@form}
        id="candidate-verify-otp-form"
        action={~p"/candidate/verify-otp"}
        method="post"
