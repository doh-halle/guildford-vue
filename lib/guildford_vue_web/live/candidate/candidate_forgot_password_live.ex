defmodule GuildfordVueWeb.Candidate.CandidateForgotPasswordLive do
  @moduledoc """
  LiveView for GET /candidate/forgot-password. The user enters their
  ...
  alias GuildfordVue.{AuthAuditLog, Candidates, Hammer}

  # 5 forgot-password attempts per IP per hour (Sprint 11.5 Slice 1) —
  # kills the user-enumeration vector.
  @rate_limit_scope "candidate-forgot-password"
  @rate_limit_limit 5
  @rate_limit_window_ms 60 * 60 * 1000

  @impl Phoenix.LiveView
  def mount(_params, _session, socket) do
    form = to_form(%{"email" => ""}, as: :candidate)
  ...
  @impl Phoenix.LiveView
  def handle_event("submit", %{"candidate" => %{"email" => email}}, socket) do
    case Hammer.check(
           @rate_limit_scope,
  ...
  defp do_submit(socket, email) do
    _ = AuthAuditLog.log_password_reset_requested(:candidate, email, Hammer.socket_ip(socket))

    if candidate = Candidates.get_candidate_by_email(email) do
      Candidates.deliver_password_reset_instructions(candidate, fn token ->
        url(socket, ~p"/candidate/reset-password/#{token}")
      end)
    end
  ...
  defp deny(socket, retry_after) do
    {:noreply,
     put_flash(
       socket,
       :error,
       "Too many requests. Please try again in #{retry_after}s."
     )}
  end
  ...
  def render(assigns) do
    ...
    <main class="mx-auto max-w-md px-6 py-16 sm:py-24">
      ...
      <.form
        for={@form}
        id="candidate-forgot-password-form"
        phx-submit="submit"
