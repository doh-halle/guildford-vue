defmodule GuildfordVueWeb.Candidate.CandidateResetPasswordLive do
  @moduledoc """
  LiveView for GET /candidate/reset-password/:token. Renders the new-
  ...
  def mount(%{"token" => token}, _session, socket) do
    case Candidates.get_candidate_by_reset_token(token) do
      nil ->
        {:ok, assign(socket, status: :invalid_token, page_title: "Invalid reset link")}

      _candidate ->
        form = to_form(%{"password" => ""}, as: :candidate)
  ...
  def handle_event("save", %{"candidate" => params}, socket) do
    case Candidates.reset_password(socket.assigns.token, params) do
      {:ok, candidate} ->
        _ =
      ...
      {:error, %Ecto.Changeset{} = changeset} ->
        ...
        {:noreply, assign(socket, form: to_form(changeset, as: :candidate))}
    ...
    <main class="mx-auto max-w-md px-6 py-16 sm:py-24">
      ...
      <%= case @status do %>
        <% :valid -> %>
          ...
          <.form
            for={@form}
            id="candidate-reset-password-form"
            phx-submit="save"
        ...
        <% :invalid_token -> %>
          ...
          <p class="mt-2 text-sm text-ink-600">
            ...
            <.link
              navigate={~p"/candidate/forgot-password"}
