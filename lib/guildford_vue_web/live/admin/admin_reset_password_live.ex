defmodule GuildfordVueWeb.Admin.AdminResetPasswordLive do
  @moduledoc """
  LiveView for GET /backoffice/reset-password/:token. Renders the new-
  ...
  def mount(%{"token" => token}, _session, socket) do
    case Admins.get_admin_by_reset_token(token) do
      nil ->
        {:ok, assign(socket, status: :invalid_token, page_title: "Invalid reset link")}

      _candidate ->
        form = to_form(%{"password" => ""}, as: :admin)
  ...
  def handle_event("save", %{"admin" => params}, socket) do
    case Admins.reset_password(socket.assigns.token, params) do
      {:ok, admin} ->
        _ = AuthAuditLog.log_password_reset_completed(:admin, admin.id, Hammer.socket_ip(socket))
      ...
      <%= case @status do %>
        ...
        <% :invalid_token -> %>
          ...
          <p class="mt-2 text-sm text-ink-600">
            ...
            <.link
              navigate={~p"/backoffice/forgot-password"}
